...
Also note, if you have already run this and have the files, then you dont need to run it again unless to update with new data.
...
# OISST is periodically updated.  Here we use v2.1 for the 2022 and 2023 reports.  Please make sure you are using the most up to date version
...
OISST_sub <- function(times, mypath){
  oisst_res <- griddap(#x = "ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon", 
    ...
    longitude = c(280,300),# E W
    fields = "sst",
    store = disk(path = mypath)
    #,read = FALSE) #Where to save it!!!
  )
    }
# NOTE the below commented out version was used through the 2021 report.  For the 2022 report OISST has moved to v2.1 (as of data May 2020 and more recent)
#####Function 1 (get data)
#OISST_sub <- function(times, mypath){
#  oisst_res <- griddap(#x = "ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon", 
#    #url = "https://www.ncei.noaa.gov/erddap/", 
...
#    #depth = c(0, 0),
#    latitude = c(32,46),# North south
#    longitude = c(-80,-60),# E W (need to subtract long in normal format from #360 to get this)
#    fields = "sst",
#    store = disk(path = mypath)
#    #,read = FALSE) #Where to save it!!!
#  )
...
#Old version, THIS link stopped working when OISST was updated
#OISST_sub <- function(times, mypath){
#  oisst_res <- griddap(x = "ncdc_oisst_v2_avhrr_by_time_zlev_lat_lon", 
#                       url = "https://www.ncei.noaa.gov/erddap/", 
#                       time = times, 
...
#NOTE THIS PATH IS NOT A FOLDER ON THE GITHUB
#Most of the raw data is too large to store on github.  This is where I located it on my local computer.  Therefore this data is stored on the NYOS megafolder in the google drive at 
#NYOS_megafolder/indicator.development/Indicators_MOST_RECENT_2023/Raw Data/Temperature

mypath<-"~/Desktop/NYB Indicators/CalculateIndicators/WaterTemperature/Data/CV/OISST_v2.1"

#####Running the following commands will take about 15 mintues
#####For the 2023 report we updated the SST for newly available data - OISST8cv

  #OISST1cv <- OISST_sub(c("1982-01-01T00:00:00Z", "1985-12-31T00:00:00Z"),mypath)
  #OISST2cv <- OISST_sub(c("1986-01-01T00:00:00Z", "1990-12-31T00:00:00Z"),mypath)
  #OISST3cv <- OISST_sub(c("1991-01-01T00:00:00Z", "1999-12-31T00:00:00Z"),mypath)
  #OISST4cv <- OISST_sub(c("2000-01-01T00:00:00Z", "2008-12-31T00:00:00Z"),mypath)
  #OISST5cv <- OISST_sub(c("2009-01-01T00:00:00Z", "2013-12-31T00:00:00Z"),mypath)
  #OISST6cv <- OISST_sub(c("2014-01-01T00:00:00Z", "2019-12-31T00:00:00Z"),mypath)
  #OISST7cv <- OISST_sub(c("2020-01-01T00:00:00Z", "2022-08-24T00:00:00Z"),mypath)
  OISST8cv <- OISST_sub(c("2022-08-25T00:00:00Z", "2023-11-05T00:00:00Z"),mypath)
```

Now you have eight files with long strange names saved in **mypath** so these will get used in the next step. I have have manually renamed the files to reflect what they are.

For the Surface 20C isotherm indicator next steps can be found in Surface20Cisotherm.ipynb
...
d1$lon<-(360-d1$lon)*-1#fix lat lon
...
###And it can be labled according to the tlabs dataframe
####These are all of the data created with GetSST

setwd("~/Desktop/NYB Indicators/CalculateIndicators/WaterTemperature/Data/CV/OISST_v2.1/")
d1<-OISST_prep("OISST_daily_v2.1_1982_1985.nc")
d2<-OISST_prep("OISST_daily_v2.1_1986_1990.nc")
d3<-OISST_prep("OISST_daily_v2.1_1991_1999.nc")
d4<-OISST_prep("OISST_daily_v2.1_2000_2008.nc")
d5<-OISST_prep("OISST_daily_v2.1_2009_2013.nc")
d6<-OISST_prep("OISST_daily_v2.1_2014_2019.nc")
d7<-OISST_prep("OISST_daily_v2.1_2020_2022jul.nc")
d8<-OISST_prep("OISST_daily_v2.1_2022jul_2023nov.nc")

ddd<-rbind(d1,d2,d3,d4,d5,d6,d7,d8)
rm(d1,d2,d3,d4,d5,d6,d7,d8)
nrow(ddd)

ddd$lon <- (360-ddd$lon)*-1#convert to correct lat lon
...
#Again this is where I have saved this on my local computer.  Please also save it to the google drive NYOS Megafolder.  This data is too big for github

setwd("~/Desktop/NYB Indicators/CalculateIndicators/WaterTemperature/Data")
write.csv(ddd, "L1_SST_data_ProcessedNOV21_2023.csv", row.names = FALSE)#takes awhile
```
