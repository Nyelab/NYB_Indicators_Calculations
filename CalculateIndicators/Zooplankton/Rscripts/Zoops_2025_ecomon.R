...
#Laura Gruenburg, Last edited June 6, 2024
...
# This ecomon most recent data is in the NYOS mgeafolder (NYOS_megafolder/indicator.development/Indicators_MOST_RECENT_2023/Raw Data)
setwd("/Users/lauragruenburg/Downloads/0187513/4.4/data/0-data/")
#ZPD=openxlsx::read.xlsx("L1_EcoMon_Plankton_Data_v3_8.xlsx", sheet='Data')
ZPD <- read.csv("EcoMon_Plankton_Data_v3_10.csv")

#reformat the data
dt=as_date(ZPD$DATE,, format = '%d-%h-%y') #origin = "1899-12-30")
...
######REORDER
ZPDb=ZPD[,c(1:14, 290:296, 106:197)] #####CAHNGED 297 to 296 - please double check each year to make sure you are picking the right columns - correct as of 2025 report
...
p.a=ZPDa[,23:113] #all the columns with the zooplankton data in it
p.a[p.a > 0]=1 # presence/absence
count=colSums(p.a, na.rm = TRUE)
...
test=log10(ZPDa[,22:48]+1)
ZPDlog=ZPDa
ZPDlog[,22:48]=test#the columns that hold zoop data
...
for(col_sep in 51:52){ ###loop over scale 1, scale 2 etc.
#  col_sep = 51#52
  print(col_sep)
  

  
  #col_sep = 22 #which column is the scale unit designation
  ...
  for(k in 1:n_areas){
    ...
    for (i in 22:48){  #22 to 49 are the columns that have zoop data
      #num=23#first taxa 
      num=i
      #num=23
      name=nm[num,1]
    ...
    colnames(gbk.yr.spln)=nm[22:48,1] #23 thrhrou 51 are the columns with zoop data
    ...
    gbk.anom=ZPDlog[which(ZPDlog$epu==areas[k]),]
    ...
    for (i in 1:dim(gbk.anom)[1]){
      ...
      gbk.anom.b[i,22:48]=gbk.anom[i,22:48]-gbk.yr.spln[which(gbk.anom$DOY[i]==rownames(gbk.yr.spln)),]
      print(i)
    }
  ...
  all_this_scale[[col_sep-50]]<-do.call('rbind',zoop_anomaly)
  print(col_sep)     
}
...
finalTSnyb<-data.frame(Year = NYB$year,
                    Variable = NYB$variable,
                    Val = NYB$value,
                    Loc = NYB$Scale2,
                    N = NYB$counts_year)
finalTSnyb$Year<-as.numeric(as.character(finalTSnyb$Year))
...
write.csv(finalTSnyb, file = "Zoops_2025nyb.csv")
