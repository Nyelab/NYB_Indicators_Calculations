...
########################################################
setwd("~/Desktop/NYB_Indicators_Calculations/Datasets/Shapefiles")
EPUshp<-st_read("EPU_NOESTUARIES.shp")#Ecological production units
NYBshp<-st_read("PlanningArea_NYocean_NYSDOS.shp")#NYB
Strata<-st_read("Strata.shp")#NYB

#transform NYB to common proj4string
NYBshp<-st_transform(NYBshp, st_crs(EPUshp))
Strata<-st_transform(Strata, st_crs(EPUshp))
#plot(EPUshp)
...
klab<-function(LAT, LON){
  ...
  pts<-SpatialPointsDataFrame(data.frame(LON, LAT),
                              data = data.frame(rowID = 1:length(LAT),LAT,LON), proj4string = 
                                as(st_crs(EPUshp), "CRS"))
  
  EPUshp_sp <- as(EPUshp, "Spatial")
  NYBshp_sp <- as(NYBshp, "Spatial")
  Strata_sp <- as(Strata, "Spatial")
  
  #DO the over function to create labels based on overlap
  mabLab<-over(pts, EPUshp_sp)
  stratLab<-over(pts, Strata_sp)
  NYBshp_sp$NYB<-"NYB"
  nybLab<-over(pts, NYBshp_sp)
