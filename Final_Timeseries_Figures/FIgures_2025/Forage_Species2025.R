...
ds<-read.csv("forage_spring_2025.csv", header = TRUE)
...
intercept = 22.79731  # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass (kg/tow)"), x = '', title = 'Forage Species') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("forage_fall_2025.csv", header = TRUE)
...
intercept2 = 27.59167    # look at p2_mod and extract the intercept
