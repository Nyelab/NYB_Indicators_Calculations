...
ds<-read.csv("trawl_spring_2025.csv", header = TRUE)
...
mn_ds5 = mean(ds$strat.biomass[ds$YEAR >= 2020])
...
intercept = 190.0776    # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  geom_point(data = ds[57, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass (kg/tow)"), x = '', title = 'Total Trawl Biomass') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))

## FALL

ds2<-read.csv("trawl_fall_2025.csv", header = TRUE)
...
intercept2 = 127.3991      # look at p2_mod and extract the intercept
...
p2 = ggplot() + 
  ...
  geom_point(data = ds2[60, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass (kg/tow)"), x = 'Year', title = '') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))
