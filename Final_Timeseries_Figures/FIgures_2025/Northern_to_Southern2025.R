...
ds_n<-read.csv("north_spring_2025.csv", header = TRUE)
ds_s<-read.csv('south_spring_2025.csv', header = TRUE)
...
mn_ds_n5 = mean(ds_n$ratio[ds_n$YEAR >= 2020])
...
intercept = 25.83869      # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  geom_point(data = ds_n[57, ], aes(x = YEAR, y = ratio), shape = 17, size = 3) + 
...
ds2_n<-read.csv("north_fall_2025.csv", header = TRUE)
ds2_s<-read.csv("south_fall_2025.csv", header = TRUE)
...
intercept2 = 5.519327    # look at p2_mod and extract the intercept
...
p2 = ggplot() + 
  ...
  geom_point(data = ds2_n[60, ], aes(x = YEAR, y = ratio), shape = 17, size = 3) + 
