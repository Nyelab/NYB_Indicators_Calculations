...
ds<-read.csv("avgtemp_spring_2025.csv", header = TRUE)
...
mn_ds5 = mean(ds$wt_mean_temp[ds$YEAR >= 2020])
...
intercept =8.518061   # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  geom_point(data = ds[57, ], aes(x = YEAR, y = wt_mean_temp), shape = 17, size = 3) + 
...
##FALL
ds2<-read.csv("avgtemp_fall_2025.csv", header = TRUE)
...
intercept2 = 10.43006    # look at p2_mod and extract the intercept
...
p2 = ggplot() + 
  ...
  geom_point(data = ds2[60, ], aes(x = YEAR, y = wt_mean_temp), shape = 17, size = 3) + 
