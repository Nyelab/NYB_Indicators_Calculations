## Figures for Surface Saliniity Seasonally
...
bsal<-read.csv("SurfSalt_insitu_Nov_8_2025.csv", header = TRUE)
...
mn_winter5 = mean(winter$Val[winter$Year >= 2013])
...
intercept_w = 32.83469   # look at p2_mod and extract the intercept
...
Term = "Year"
mod_w.d <- Deriv(mod_w, n=32) # n is the number of years
...
# find the last 5 years mean
mn_spring5 = mean(spring$Val[spring$Year >= 2020])
# what quintile the data is in
mn_spring5 >qspring
...
intercept_sp = 32.80156   # look at p2_mod and extract the intercept
pdata_sp <- transform(pdata_sp, p2_mod_sp = p2_mod_sp$fit[,1], se2_sp = p2_mod_sp$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "Year"
mod_sp.d <- Deriv(mod_sp, n=44) # n is the number of years
...
intercept_su = 31.73638   # look at p2_mod and extract the intercept
...
# find the last 5 years mean
mn_fall5 = mean(fall$Val[fall$Year >= 2021])
# what quintile the data is in
mn_fall5 >qfall
...
intercept_fa = 32.54249   # look at p2_mod and extract the intercept
pdata_fa <- transform(pdata_fa, p2_mod_fa = p2_mod_fa$fit[,1], se2_fa = p2_mod_fa$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "Year"
mod_fa.d <- Deriv(mod_fa, n=43) # n is the number of years
...
fall_plot <- ggplot() + 
  ...
  geom_point(data = fall[43,], aes(x= Year, y = Val_I), shape = 17, size =3) +
  geom_smooth(data = fall, aes(x = Year, y = Val_I), method = lm, se = FALSE, color = 'black') + 
