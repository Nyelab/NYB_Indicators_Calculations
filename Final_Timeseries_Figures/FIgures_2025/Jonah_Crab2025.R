...
ds_1<-read.csv("jonah_spring_2025.csv", header = TRUE)
...
intercept = 0.07465437   # look at p2_mod and extract the intercept
...
Term = "YEAR"
mod.d <- Deriv(mod, n=47) # n is the number of YEARs
...
p1 = ggplot() + 
  ...
  geom_point(data = ds[47, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass (kg/tow)"), x = '', title = 'Jonah Crab') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds_2<-read.csv("jonah_fall_2025.csv", header = TRUE)
...
intercept2 = 0.112636    # look at p2_mod and extract the intercept
