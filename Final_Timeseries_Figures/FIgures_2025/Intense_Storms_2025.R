...
Lstorms<-read.csv("Cyclones_2025.csv", header = TRUE)
...
mn_AI5 = mean(AI$activity_index[AI$year >= 2020])
...
intercept_w = 129.1212 # look at p2_mod and extract the intercept
...
Term = "year"
mod_w.d <- Deriv(mod_w, n=85) # n is the number of years
...
cia_plot <- ggplot() + 
  ...
  geom_point(data = AI[85,], aes(x= year, y = activity_index), shape = 17, size =3) +
  ...
  theme_bw() +
  labs (y = bquote("Activity Index"), x =' ',title = 'Activity Index') + 
  theme(plot.title=element_text(size = 20,face = 'bold',hjust = 0.5), axis.title=element_text(size = 18, face = 'bold'), axis.text= element_text(color = 'black', size = 16))
...
# find the last 5 years mean
mn_Large5 = mean(Large$n[Large$year >= 2020])
# what quintile the data is in
mn_Large5 >qLarge
...
intercept_sp = 8.4 # look at p2_mod and extract the intercept
pdata_sp <- transform(pdata_sp, p2_mod_sp = p2_mod_sp$fit[,1], se2_sp = p2_mod_sp$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "year"
mod_sp.d <- Deriv(mod_sp, n=85) # n is the number of years
...
Str_plot <- ggplot() + 
  ...
  geom_point(data = Large[85,], aes(x= year, y = n), shape = 17, size =3) +
  ...
  theme_bw() +
  labs (y = bquote("Intense Storms per Year"), x = ' ', title = 'Number of Intense Storms') + 
  theme(plot.title=element_text(size = 20,face = 'bold',hjust = 0.5), axis.title=element_text(size = 18, face = 'bold'), axis.text= element_text(color = 'black', size = 16))
