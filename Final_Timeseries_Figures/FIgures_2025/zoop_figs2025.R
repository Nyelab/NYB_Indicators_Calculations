...
zoops<-read.csv("Zoops_2025nyb.csv", header = TRUE)
...
intercept = -0.1434994   # look at p2_mod and extract the intercept
...
ggplot() + 
  ...
  theme_bw() +
  labs (y = bquote("Abundance Anomaly"), x = 'Year', title = ' ') + 
...
p2_mod <- predict(mod, newdata = pdata,  type = "terms", se.fit = TRUE)
intercept = 0.1115534   # look at p2_mod and extract the intercept
pdata <- transform(pdata, p2_mod = p2_mod$fit[,1], se2 = p2_mod$se.fit[,1])
...
p2_mod <- predict(mod, newdata = pdata,  type = "terms", se.fit = TRUE)
intercept = 0.1456816  # look at p2_mod and extract the intercept
pdata <- transform(pdata, p2_mod = p2_mod$fit[,1], se2 = p2_mod$se.fit[,1])
