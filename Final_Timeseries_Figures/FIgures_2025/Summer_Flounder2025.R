...
ds<-read.csv("summerflounder_spring_2025.csv", header = TRUE)
...
intercept = 1.263735    # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  geom_point(data = ds[55, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass (kg/tow)"), x = '', title = 'Summer Flounder') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))

## FALL
ds2<-read.csv("summerflounder_fall_2025.csv", header = TRUE)
...
intercept2 = 1.015857     # look at p2_mod and extract the intercept
