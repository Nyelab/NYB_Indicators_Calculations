...
Benthos<-read.csv("benthos_spring_2025.csv", header = TRUE)
Benthos
...
mn_Benthos5 = mean(Benthos$strat.biomass[Benthos$YEAR >= 2020])
...
intercept_cr = 1.190644   # look at p2_mod and extract the intercept
...
Term = "YEAR"
mod_cr.d <- Deriv(mod_cr, n=57) # n is the number of YEARs
...
Bt <- ggplot() + 
  ...
  geom_point(data = Benthos[57,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
  ...
  theme_bw() +
  labs (y = "Biomass (kg/tow)", x = '', title = 'Spring') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))

### Benthos Fall
Benthos_f<-read.csv("benthos_fall_2025.csv", header = TRUE)
Benthos_f
...
intercept_bf = 3.046868   # look at p2_mod and extract the intercept
pdata_bf <- transform(pdata_bf, p2_mod_bf = p2_mod_bf$fit[,1], se2 = p2_mod_bf$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_bf.d <- Deriv(mod_bf, n=60) # n is the number of YEARs
...
Bt_f <- ggplot() + 
  ...
  geom_point(data = Benthos_f[60,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
  ...
  theme_bw() +
  labs (y = "Biomass (kg/tow)", x = '', title = 'Fall') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))
#
...
Benthivore<-read.csv("benthivore_spring_2025.csv", header = TRUE)
Benthivore

qBenthivore = quantile(Benthivore$strat.biomass, probs = c(.30, .70))

# find the last 5 years mean
mn_Benthivore5 = mean(Benthivore$strat.biomass[Benthivore$YEAR >= 2020])
# what quintile the data is in
...
intercept_bvs = 13.60731   # look at p2_mod and extract the intercept
pdata_bvs <- transform(pdata_bvs, p2_mod_bvs = p2_mod_bvs$fit[,1], se2 = p2_mod_bvs$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_bvs.d <- Deriv(mod_bvs, n=57) # n is the number of YEARs
...
Bvs <- ggplot() + 
  ...
  geom_point(data = Benthivore[57,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
...
### Benthivore Fall
Benthivore_f<-read.csv("Benthivore_fall_2025.csv", header = TRUE)
Benthivore_f
...
intercept_bvf = 19.69168   # look at p2_mod and extract the intercept
pdata_bvf <- transform(pdata_bvf, p2_mod_bvf = p2_mod_bvf$fit[,1], se2 = p2_mod_bvf$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_bvf.d <- Deriv(mod_bvf, n=60) # n is the number of YEARs
...
Bv_f <- ggplot() + 
  ...
  geom_point(data = Benthivore_f[60,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
...
Planktivore<-read.csv("Planktivore_spring_2025.csv", header = TRUE)
Planktivore

qPlanktivore = quantile(Planktivore$strat.biomass, probs = c(.30, .70))

# find the last 5 years mean
mn_Planktivore5 = mean(Planktivore$strat.biomass[Planktivore$YEAR >= 2020])
# what quintile the data is in
...
intercept_pvs = 25.29151   # look at p2_mod and extract the intercept
pdata_pvs <- transform(pdata_pvs, p2_mod_pvs = p2_mod_pvs$fit[,1], se2 = p2_mod_pvs$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_pvs.d <- Deriv(mod_pvs, n=57) # n is the number of YEARs
...
Pvs <- ggplot() + 
  ...
  geom_point(data = Planktivore[57,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
...
### Planktivore Fall
Planktivore_f<-read.csv("Planktivore_fall_2025.csv", header = TRUE)
Planktivore_f
...
intercept_pvf = 27.52134   # look at p2_mod and extract the intercept
pdata_pvf <- transform(pdata_pvf, p2_mod_pvf = p2_mod_pvf$fit[,1], se2 = p2_mod_pvf$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_pvf.d <- Deriv(mod_pvf, n=60) # n is the number of YEARs
...
Pv_f <- ggplot() + 
  ...
  geom_point(data = Planktivore_f[60,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
...
Piscivore<-read.csv("Piscivore_spring_2025.csv", header = TRUE)
Piscivore

qPiscivore = quantile(Piscivore$strat.biomass, probs = c(.30, .70))

# find the last 5 years mean
mn_Piscivore5 = mean(Piscivore$strat.biomass[Piscivore$YEAR >= 2020])
# what quintile the data is in
...
intercept_pcs =  148.5318    # look at p2_mod and extract the intercept
pdata_pcs <- transform(pdata_pcs, p2_mod_pcs = p2_mod_pcs$fit[,1], se2 = p2_mod_pcs$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_pcs.d <- Deriv(mod_pcs, n=57) # n is the number of YEARs
...
Pcs <- ggplot() + 
  ...
  geom_point(data = Piscivore[57,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
...
### Piscivore Fall
Piscivore_f<-read.csv("Piscivore_fall_2025.csv", header = TRUE)
Piscivore_f
...
intercept_pcf = 68.48979    # look at p2_mod and extract the intercept
pdata_pcf <- transform(pdata_pcf, p2_mod_pcf = p2_mod_pcf$fit[,1], se2 = p2_mod_pcf$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_pcf.d <- Deriv(mod_pcf, n=60) # n is the number of YEARs
...
Pc_f <- ggplot() + 
  ...
  geom_point(data = Piscivore_f[60,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
