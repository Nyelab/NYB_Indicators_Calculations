...
mhw_ss<-read.csv("mhw_2025_surf_static.csv", header = TRUE)
mhw_bs<-read.csv("mhw_2025_bot_static.csv", header = TRUE)

mhwdays = mhw_ss[mhw_ss$Variable == "OISST_HW_total_days", ]
mhwdays = mhwdays[mhwdays$Loc == "NYB", ]
mhwdays$Val[is.na(mhwdays$Val)] <- 0

qmhwdays = quantile(mhwdays$Val, probs = c(0.30, 0.70), na.rm = TRUE)

# find the last 5 years mean
mn_mhwdays5 = mean(mhwdays$Val[mhwdays$Year >= 2021])
...
intercept_s = 59.31111  # look at p2_mod and extract the intercept_s
...
Term = "Year"
mod_s.d <- Deriv(mod_s, n=45) # n is the number of years
...
surf_plot <- ggplot() + 
  ...
  geom_point(data = mhwdays[45,], aes(x = Year, y = Val), shape = 17, size = 3) +
...
mhwdays2 = mhw_bs[mhw_bs$Variable == "GLORYS_HW_total_days", ]
mhwdays2 = mhwdays2[mhwdays2$Loc == "NYB", ]

mhwdays2$Val[is.na(mhwdays2$Val)] <- 0

qmhwdays2 = quantile(mhwdays2$Val, probs = c(.30,.70), na.rm = TRUE)

# find the last 5 years mean
mn_mhwdays25 = mean(mhwdays2$Val[mhwdays2$Year >= 2021], na.rm = TRUE)
# what quintile the data is in
...
intercept2 = 76.63636   # look at p2_mod and extract the intercept
pdata2 <- transform(pdata2, p2_mod2 = p2_mod2$fit[,1], se2 = p2_mod2$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "Year"
mod2.d <- Deriv(mod2, n=33) # n is the number of years
...
bottom_plot<- ggplot() + 
  ...
  geom_point(data = mhwdays2[33,], aes(x = Year, y = Val), shape = 17, size = 3) +
...
#####
count <- mhw_ss[mhw_ss$Variable == 'OISST_HW_count',]
...
surf_mov<-read.csv("mhw_surf_2025_moving.csv", header = TRUE)
bot_mov<-read.csv("mhw_bot_2025_moving.csv", header = TRUE)

surf_mov <- surf_mov[surf_mov$Variable == "OISST_HW_moving_total_days", ]
bot_mov <- bot_mov[bot_mov$Variable == "GLORYS12_moving_total_days", ]
bot_mov$Val[is.na(bot_mov$Val)] <- 0
