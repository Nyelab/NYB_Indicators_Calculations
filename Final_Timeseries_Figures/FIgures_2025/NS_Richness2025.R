...
ds<-read.csv("Nrichness_spring_2025.csv", header = TRUE)
...
intercept = 25.7193   # look at p2_mod and extract the intercept
...
Term = "YEAR"
mod.d <- Deriv(mod, n=57) # n is the number of YEARs
...
p1 = ggplot() + 
  ...
  geom_point(data = ds[57, ], aes(x = YEAR, y = n), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Number of Species"), x = '', title = 'Fish Northern Species Richness') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))

# fall
ds2<-read.csv("Nrichness_fall_2025.csv", header = TRUE)
...
intercept2 = 23.38333    # look at p2_mod and extract the intercept
pdata2 <- transform(pdata2, p2_mod2 = p2_mod2$fit[,1], se2 = p2_mod2$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod2.d <- Deriv(mod2, n=60) # n is the number of YEARs
...
p2 = ggplot() + 
  ...
  geom_point(data = ds2[60, ], aes(x = YEAR, y = n), shape = 17, size = 3) + 
...
ds<-read.csv("Srichness_spring_2025.csv", header = TRUE)
...
pdata <- with(ds, data.frame(YEAR = YEAR))
p2_mod <- predict(mod, newdata = pdata,  type = "terms", se.fit = TRUE)
intercept = 19.57895   # look at p2_mod and extract the intercept
pdata <- transform(pdata, p2_mod = p2_mod$fit[,1], se2 = p2_mod$se.fit[,1])
...
p1 = ggplot() + 
  ...
  geom_line(data = pdata, aes(y = unlist(mod.dsig$decr)+intercept, x = YEAR), color = 'red', size = 1) + 
  theme_bw() +
  labs (y = bquote("Number of Species"), x = '', title = 'Fish Southern Species Richness') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))

# fall
ds2<-read.csv("Srichness_fall_2025.csv", header = TRUE)
...
pdata2 <- with(ds2, data.frame(YEAR = YEAR))
p2_mod2 <- predict(mod2, newdata = pdata2,  type = "terms", se.fit = TRUE)
intercept2 = 25.66667    # look at p2_mod and extract the intercept
pdata2 <- transform(pdata2, p2_mod2 = p2_mod2$fit[,1], se2 = p2_mod2$se.fit[,1])
