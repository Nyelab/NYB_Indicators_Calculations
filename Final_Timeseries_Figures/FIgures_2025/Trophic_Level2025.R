...
ds<-read.csv("avgtl_spring_2025.csv", header = TRUE)
...
mn_ds5 = mean(ds$TL[ds$YEAR >= 2020])
...
intercept = 4.076711  # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  geom_point(data = ds[57, ], aes(x = YEAR, y = TL), shape = 17, size = 3) + 
...
##FALL
ds2<-read.csv("avgtl_fall_2025.csv", header = TRUE)
...
intercept2 = 3.926142    # look at p2_mod and extract the intercept
...
p2 = ggplot() + 
  ...
  geom_point(data = ds2[60, ], aes(x = YEAR, y = TL), shape = 17, size = 3) + 
